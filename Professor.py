...
class Professor:
    def __init__(self):
        self.dic_add_dados = {}
        self.lista_diciplina = []
        
    def cad_professor(self, id, user, password, telefone, email):
        self.id = id
        nome_valido = Validar_nome.validar_nome(user)
        password_valido = Validar_senha.validar_senha(password)
        tel_valido = Validar_telefone.validar_telefone(telefone)
            ...
            if tel_valido:
                if email_valido:
                    if password_valido:
                        ...
                        self.dic_add_dados['tel'] = tel_valido
                        self.dic_add_dados['email'] = email_valido
                        while True:
                            diciplina = str(input("diciplina: "))
                            self.lista_diciplina.append(diciplina)
                            continuar = int(input("continuar? 1-sim 2-não: "))

                            if continuar != 1:
                                break
                        self.dic_add_dados['diciplina'] = self.lista_diciplina
                        self.dic_add_dados['funcao'] = "professor"
...
def cad_professor():
    while True:
        ...
        professor = Professor()
        professor.cad_professor(id, nome, password, telefone, email)
        print("\n")
